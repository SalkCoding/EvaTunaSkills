...
val swordM = Skill(
        ...
        listOf("${ChatColor.LIGHT_PURPLE}검으로 공격시 추가적으로 대상을 벱니다.",
                "${ChatColor.LIGHT_PURPLE}스킬 레벨이 높아질수록 베는 횟수가 증가합니다."), 3, Material.GOLDEN_SWORD * 1
)

val axeM = Skill(
        "${ChatColor.GOLD}${ChatColor.BOLD}강타", SkillCategory.COMBAT,
        listOf("${ChatColor.LIGHT_PURPLE}도끼로 공격시 일정확률로 대상에게 폭발을 일으킵니다.",
                "${ChatColor.LIGHT_PURPLE}스킬 레벨이 높아질수록 폭발확률이 증가합니다."), 3, Material.GOLDEN_AXE * 1
)
...
val bodyM = Skill(
        ...
        listOf("${ChatColor.LIGHT_PURPLE}스킬 레벨에 비례하여 영구적으로 체력이 증가합니다.","${ChatColor.RED}5 레벨 마다 스킬레벨을 올릴 수 있습니다."), 5, Material.GOLDEN_CHESTPLATE * 1
)
